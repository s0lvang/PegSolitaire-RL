...
class Agent():
    ...
    def runEpisodes(self, numberOfEpisodes):
        ...
        for _ in range(numberOfEpisodes):
            ...
            state = enviroment.board.getState() # the board represented in a bitstring maybe initalize as None
            action = None # No action should be done initially.
            SAPpairs = []
            while not enviroment.isEndState():
                newState, reinforcement = enviroment.board.move(action) #I think the reinforcement should be the number of Pegs left if there is the endstate is reached. else 0
                if (enviroment.isEndState()):
                    print(reinforcement)
                    pegsLeft.append(reinforcement)
                    break
                newAction = self.actor.chooseAction(newState) # The article about reinforcement learning, just states that the actor chooses an action, but i think it should know which actions are legal. 
                self.actor.updateEligibility(newState, newAction, isCurrentState=True) # This should update the eligibility of the SAP to 1, but that will be handled in the function
